      ...
      var i = {
        ...
        a = function (e) {
          e || (e = n.triggerElement),
            !0 !== m(e) &&
              ("dropdown" === v(e) ? U(e) : "accordion" === v(e) && A(e),
              KTUtil.data(e).set("type", v(e)));
        },
        l = function (e) {
          e || (e = n.triggerElement),
            !1 !== m(e) &&
              ("dropdown" === v(e) ? w(e) : "accordion" === v(e) && I(e));
        },
        s = function (e) {
          if (!1 !== f(e)) {
            var t = g(e);
...
// SETTING LOGIN USER/ADMIN DETAILS
var KTUserMenu = (function () {
  var updateUserMenu = function () {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user) {
      // Set name and role
      const usernameElement = document.querySelector(".usermenu");
      const nameSpan = document.querySelector(".usermenu .header_user-name");
      if (usernameElement) {
        nameSpan.textContent = user.name || "User";
      }

      //Set Email
      const emailElement = document.querySelector(".header_email");
      if (emailElement) {
...
// SIDEBAR THEME CHANGES LOGIC
const KTThemeSidebarSwitcher = (() => {
  ...
  const updateSidebarTheme = (theme) => {
    if (!body || !logoImg) return;

    // Update data attribute
    body.setAttribute("data-kt-app-layout", `${theme}-sidebar`);
  ...
  const restoreSidebarTheme = () => {
    const storedTheme = localStorage.getItem("sidebar-theme") || "dark";
    updateSidebarTheme(storedTheme);
  };

  const handleClickEvents = () => {
    document.querySelectorAll("[data-sidebar-theme]").forEach((item) => {
      item.addEventListener("click", (e) => {
        e.preventDefault();
        const theme = item.getAttribute("data-sidebar-theme");
